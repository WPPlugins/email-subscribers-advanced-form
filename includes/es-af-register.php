...
class es_af_registerhook {
	...
	public static function es_af_adminmenu() {
		if (is_admin() && is_plugin_active('email-subscribers/email-subscribers.php') )	{
			add_submenu_page('email-subscribers', __( 'Group Selector', ES_AF_TDOMAIN ), 
...
class es_af_form_submuit {
	...
	public static function es_af_formdisplay($form_setting = array()) {
		...
		$es_af .= '<form method="post" action="' . esc_url( $_SERVER['REQUEST_URI'] ) . '">';
		...
		if($es_af_email == "YES") {
			$es_af .= '<p>';
				...
				$es_af .= '<br>';
				$es_af .= '<span class="es_af_css_txt">';
					$es_af .= '<input class="es_af_tb_css" name="es_af_txt_em" id="es_af_txt_em" value="" maxlength="225" type="text">';
				$es_af .= '</span>';
				$es_af .= $es_af_alt_em;
			$es_af .=  '</p>';
		}
